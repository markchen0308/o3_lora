...
void HW_RTC_IrqHandler(void)
{
  ...
  if (__HAL_RTC_ALARM_GET_IT_SOURCE(hrtl, RTC_IT_ALRA) != RESET)
  {
    /* Get the pending status of the AlarmA Interrupt */
    if (__HAL_RTC_ALARM_GET_FLAG(hrtl, RTC_FLAG_ALRAF) != RESET)
    {
      /* Clear the AlarmA interrupt pending bit */
      __HAL_RTC_ALARM_CLEAR_FLAG(hrtl, RTC_FLAG_ALRAF);
      /* AlarmA callback */
      HAL_RTC_AlarmAEventCallback(hrtl);
    }
  }
}
